...
'CREATE TABLE IF NOT EXISTS odds (id INTEGER PRIMARY KEY AUTOINCREMENT, event_id INTEGER NOT NULL, bookmaker TEXT NOT NULL, market TEXT NOT NULL, selection TEXT NOT NULL, odd REAL NOT NULL, updated_at TEXT DEFAULT CURRENT_TIMESTAMP, UNIQUE(event_id, bookmaker, market, selection), FOREIGN KEY(event_id) REFERENCES events(id))',
...
INDEXES = ["CREATE INDEX IF NOT EXISTS idx_events_start_time ON events(start_time)","CREATE INDEX IF NOT EXISTS idx_odds_event_market ON odds(event_id, market)","CREATE INDEX IF NOT EXISTS idx_odds_bookmaker ON odds(bookmaker)","CREATE INDEX IF NOT EXISTS idx_betfair_event_market ON betfair_odds(event_id, market)","CREATE INDEX IF NOT EXISTS idx_opportunities_type ON opportunities(type)","CREATE INDEX IF NOT EXISTS idx_opportunities_roi ON opportunities(roi)","CREATE INDEX IF NOT EXISTS idx_opportunities_ql ON opportunities(qualifying_loss)"]
def init_db():
